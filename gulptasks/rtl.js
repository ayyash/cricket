...
var transform = require('gulp-transform'); // install this for rtl
var config = require(__dirname + '/config.json');


gulp.task('rtl:default', ['less:rtl'], function () {
    // Watch .sass files
    gulp.watch([config.srcUrl + 'less/*.less'], ['less:rtl']);

});


// rtl tasks
gulp.task('less:rtl', ['rawless'], function () {
	// grab less and shut rtl.sh.less and any local rtl.*.less,  join and mirror into dev

	return gulp.src([config.srcUrl + 'less/all.less',
            config.shutUrl + 'less/rtl.sh.less',
            config.srcUrl + 'less/rtl.*.less'])
		...
		.pipe(rename({ basename: config.projectName , suffix: '.rtl' }))
